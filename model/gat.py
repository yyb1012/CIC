...
class GAT(nn.Module):
    ...
    def forward(self, g, input_feature, return_hidden=False, get_attention=False, attention_layer=-1):
        h = input_feature
        hidden_list = []
        attn = None
        n_layers = self.n_layers
        chosen_layer = (n_layers - 1) if attention_layer == -1 else int(attention_layer)
        for layer in range(self.n_layers):
            if get_attention and layer == chosen_layer:
                h, attn = self.gats[layer](g, h, get_attention=True)
            else:
                h = self.gats[layer](g, h)
            hidden_list.append(h)
        if return_hidden:
            return (self.head(h), hidden_list, attn) if get_attention else (self.head(h), hidden_list)
        else:
            return (self.head(h), attn) if get_attention else self.head(h)
