...
class GMAEModel(nn.Module):
    ...
    @torch.no_grad()
    def node_reconstruction_error(self, g) -> torch.Tensor:
        """
        Compute node-level reconstruction error in [0, 1] for MultiSourceFusion.

        Returns:
            recon_error: [n_nodes] (larger means more anomalous)
        """
        x = g.ndata['attr'].to(g.device)
        ...
        if self.loss_fn == "sce":
            ...
            r_n = F.normalize(recon, p=2, dim=-1)
            err = (1.0 - (x_n * r_n).sum(dim=-1)).clamp(min=0.0).pow(self.alpha_l)
        else:
